...
export default function StandardPulseDetail() {
  ...
  const standard = useMemo(() => buildStandardsPulse(STANDARDS, sequences).find((item) => item.code === standardCode), [sequences, standardCode]);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 pb-8 pt-1">
      ...
      <section aria-labelledby="coverage-history-heading"><div><h2 id="coverage-history-heading" className="text-sm font-bold text-slate-950">Sequence history</h2><p className="mt-1 text-sm text-slate-600">Coverage appears here only when a sequence is completed.</p></div>{loading ? <div className="mt-3 rounded-xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-600">Loading history…</div> : standard.history.length ? <div className="mt-3 space-y-3">{standard.history.map((entry) => <Link key={entry.sequenceId} to={`/command-center/sequences/${entry.sequenceId}`} className="group block rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-violet-300 hover:shadow-md"><div className="flex items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold text-slate-950">{entry.sequenceTitle}</h3><span className="rounded-full bg-violet-50 px-2 py-0.5 text-xs font-semibold text-violet-700">{coverageLabel(entry.coverageLevel)}</span>{entry.needsRevisit && <span className="rounded-full bg-amber-50 px-2 py-0.5 text-xs font-semibold text-amber-900">Needs revisit</span>}</div>{entry.outcome && <p className="mt-2 text-sm text-slate-600">Outcome: {entry.outcome}</p>}{entry.note && <p className="mt-2 text-sm leading-6 text-slate-700">{entry.note}</p>}{entry.sequenceReflection && <p className="mt-2 text-xs leading-5 text-slate-500">Sequence reflection: {entry.sequenceReflection}</p>}</div><ArrowRight className="mt-1 h-4 w-4 text-slate-400 transition group-hover:translate-x-0.5 group-hover:text-violet-700" /></div></Link>)}</div> : <div className="mt-3 rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center text-sm leading-6 text-slate-600">No completed sequence has reflected this standard yet.</div>}</section>
    </div>
  );
}
